...
class EmacsAdapterTest < Minitest::Test
  ...
  def test_modern_font_lock_roles_have_visible_hierarchy
    assert_includes @content, "'(font-lock-property-use-face ((t (:foreground \"#{@theme.primary}\" :weight semi-bold))))"
    assert_includes @content, "'(font-lock-string-face ((t (:foreground \"#{@theme.secondary}\"))))"
    assert_includes @content, "'(font-lock-constant-face ((t (:foreground \"#{@theme.strong}\" :weight bold))))"
  end

  def test_active_mode_line_inherits_the_base_face
    assert_includes @content, "'(mode-line-active ((t (:inherit mode-line))))"
    refute_match(/mode-line-inactive.*:box/, @content)
  end
  ...
  def test_theme_uses_only_lacold_and_terminal_white_colors
    allowed = (@theme.neutrals.values + @theme.accent.values + ["#FFFFFF"]).uniq
