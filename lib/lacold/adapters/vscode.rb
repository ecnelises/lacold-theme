    ...
    class VSCode < Base
      ...
      def render(themes)
        ...
        files.concat([
          ...
          output("vscode/CHANGELOG.md", File.read(File.join(PROJECT_ROOT, "CHANGELOG.md"))),
          output("vscode/LICENSE.txt", File.read(File.join(PROJECT_ROOT, "LICENSE")))
        ])
        files << Output.new(
          "vscode/images/lacold-icon.png",
          File.binread(File.join(PROJECT_ROOT, "assets/lacold-icon.png"))
        )
      end
      ...
      def vscode_readme(themes)
        ...
        <<~MARKDOWN
          ...
          Open **Preferences: Color Theme** and select a Lacold theme after installation.

          Source, release archives, and issue tracking are available in the
          [Lacold repository](#{REPOSITORY_URL}).
        MARKDOWN
      end
    end
  end
